 ...
 * 
 * This file handles all authentication-related UI interactions including:
...
/**
 * Restore Google Sign-in button with icon
 * @param {HTMLElement} button - The button element
 */
...
async function handleGoogleSignIn(event) {
    ...
    try {
        // Show loading state
        setButtonLoading(googleSigninBtn, 'Signing in with Google...');
        ...
        if (!result.success) {
            ...
            if (result.error && result.error.includes('cancelled')) {
                ...
                showAlert('Sign-in was cancelled. Click the button when you\'re ready to continue.', 'error');
            } else {
                // Other errors (popup blocked, network issues, etc.)
                console.error('❌ Sign-in failed:', result.error);
                showAlert(result.error || 'Google sign-in failed. Please try again.', 'error');
            }
            restoreGoogleButton(googleSigninBtn);
            return;
        }
        ...
        showAlert(error.message || 'Failed to sign in with Google. Please try again.', 'error');
        restoreGoogleButton(googleSigninBtn);
    }
}

// ========== User ID/Password Login Handler ==========

/**
 * Handle traditional user ID and password login
 * @param {Event} event - The form submit event
 */
async function handleUserIdLogin(event) {
    ...
    try {
        ...
        showAlert(error.message || 'Failed to login. Please check your credentials.', 'error');
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🎯 Auth page loaded, initializing event listeners...');
    ...
    // Setup User ID login form
    const loginForm = document.getElementById('login-form');
    if (loginForm) {
        loginForm.addEventListener('submit', handleUserIdLogin);
    ...
    console.log('✅ All auth event listeners initialized successfully');
});
